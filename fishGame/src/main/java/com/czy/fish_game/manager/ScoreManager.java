...
import com.czy.fish_game.model.*;
...
//				effect.playEffect((float)(ammoRedius*Math.cos(Math.toRadians(angle+180))),-(float)(ammoRedius*Math.sin(Math.toRadians(angle+180))),goldFromX-gold.getPicWidth()/2, goldFromY-gold.getPicHeight()/2, x, y);
	...
	public void init(){
		try {
			...
			//初始化金币
			initGold(ImageManager.getImageMnagaer().getImagesMapByImageConfig(ImageManager.getImageMnagaer().createImageConfigByPlist("image/score/goldItem"),ImageManager.getImageMnagaer().scaleNum));
			//初始化高分
			initHighPoint(ImageManager.getImageMnagaer().getImagesMapByImageConfig(ImageManager.getImageMnagaer().createImageConfigByPlist("image/score/highPoint"),ImageManager.getImageMnagaer().scaleNum));
			//初始化百分
			initHundredPoint(ImageManager.getImageMnagaer().getImagesMapByImageConfig(ImageManager.getImageMnagaer().createImageConfigByPlist("image/score/hundred"),ImageManager.getImageMnagaer().scaleNum));
		} catch (Exception e) {
			LogTools.doLogForException(e);
		}
	}
