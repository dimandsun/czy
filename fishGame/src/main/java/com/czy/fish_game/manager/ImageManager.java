...
//	 */
	...
	public ImageConfig createImageConfigByPlist(
		...
		try {
			Object xml = XmlManager.getXmlParser(configFileName, "UTF-8");
			if(xml==null){
	 ...
	 * @param config
	 */
	private void setScaleInfo(Object xml,ImageConfig config){
		XmlManager.gotoTagByTagName(xml, "key");
...
//					this.baseImageCache.recycle();
	 ...
	 */
	private void getCutImageInfo(Object xml,
			ImageConfig config) {
		...
		while (GamingInfo.getGamingInfo().isGaming()&&XmlManager.gotoTagByTagName(xml, "key")) {
			...
			if (XmlManager.gotoTagByTagName(xml, "dict")) {
				ImageConfig.ActConfig actConfig = getActConfig(xml);
...
//				getFishActImage(xml, src);
	 ...
	 * @return
	 */
	private ImageConfig.ActConfig getActConfig(Object xml){
		ImageConfig.ActConfig imageConfig = new ImageConfig.ActConfig();
		//找到相应配置信息
		XmlManager.gotoTagByTagName(xml, "integer");
	 ...
	 * @return			裁出来的图
	 */
	private Bitmap getImage(ImageConfig.ActConfig config, Bitmap src, float proportion) {
		// 创建一个图片
	 ...
	 * @return
	 */
	public Bitmap[] getImagesByActConfigs(ImageConfig.ActConfig[] configs, float proportion){
	...
	public HashMap<String,Bitmap> getImagesMapByImageConfig(ImageConfig config,float proportion){
		...
		try{
			...
			//将所有配置信息中的图片放置到集合中
			for(ImageConfig.ActConfig act : config.getAllActs().values()){
