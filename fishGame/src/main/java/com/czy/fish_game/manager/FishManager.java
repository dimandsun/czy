...
import com.czy.fish_game.model.FishInfo;
...
public class FishManager {
	 ...
	 * 根据名字保存所有鱼的动作配置信息
	 */
	private HashMap<String, ImageConfig.ActConfig[]> allFishActConfigs = new HashMap<String, ImageConfig.ActConfig[]>();
	/**
	 * 根据名字保存所有鱼的捕获动作配置信息
	 */
	private HashMap<String, ImageConfig.ActConfig[]> allFishCatchActConfigs = new HashMap<String, ImageConfig.ActConfig[]>();
	/**
	 * 根据名字缓存的鱼的动作图片
	 */
	...
	public void initFish(){
		try{			
			HashMap<String, ImageConfig.ActConfig> configs = new HashMap<String, ImageConfig.ActConfig>();
			//创建一个鱼动作配置文件集合
			String fishActConfiges[]; 
			//鱼的基本信息配置文件名
			String fishInfoConfig;
			//加载管理器主配置文件
			Object xml = XmlManager.getXmlParser("image/fish/FishConfig", "UTF-8");
			//获取fishActConfig信息
			...
			while(getFishByName(fishName.toString(),configs)){
				...
				if(fishIndex<10){
					...
					fishName.append("image/fish" +fishIndex);
				}
			}
			configs = null;
			fishName = null;
	 ...
	 */
	private boolean getFishByName(String fishName,HashMap<String, ImageConfig.ActConfig> configs){
		//鱼的图全名(fish12_01.png)
		...
		int fishNum = 1;
		//临时存放当前鱼所有图片的集合
		ArrayList<ImageConfig.ActConfig> allActs = new ArrayList<ImageConfig.ActConfig>();
		//临时存放当前鱼所有被捕获图片的集合
		ArrayList<ImageConfig.ActConfig> allCatchActs = new ArrayList<ImageConfig.ActConfig>();
		//获取当前鱼的所有动作
		while(GamingInfo.getGamingInfo().isGaming()){
			fishFullName.delete(0, fishFullName.length());
			//一帧图片的引用
			ImageConfig.ActConfig fishAct = null;
			if(fishNum<10){
		...
		while(GamingInfo.getGamingInfo().isGaming()){
			...
			//一帧图片的引用
			ImageConfig.ActConfig fishCatchAct = null;
			if(fishNum<10){
		...
		if(allActs.size()==0){
			...
			ImageConfig.ActConfig[] fishActArray = new ImageConfig.ActConfig[allActs.size()];
			ImageConfig.ActConfig[] fishCatchActsArray = new ImageConfig.ActConfig[allCatchActs.size()];
	...
	private void initFishInfo(String config){
		try{
			...
			//加载鱼的基本信息配置文件
			Object xml = XmlManager.getXmlParser(config, "UTF-8");
			//解析所有的鱼的基本信息
	 ...
	 */
	private void initFishAct(HashMap<String, ImageConfig.ActConfig> configs, String fishActConfiges[]){
		try{
			//如果配置信息没有找到，抛出异常
