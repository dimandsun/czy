...
import model.Ammo;
import model.FishingNet;
import model.GamingInfo;
import model.WaterRipple;
import model.componets.Cannon;
import model.componets.ChangeCannonEffect;
import threads.ShotThread;
...
public class CannonManager {
	...
	public void init(){
		try {
			//获取配置文件指定的所有图片
			HashMap<String,Bitmap> allImage = manager.ImageManager.getImageMnagaer().getImagesMapByImageConfig(manager.ImageManager.getImageMnagaer().createImageConfigByPlist("cannon/bulletandnet"), manager.ImageManager.getImageMnagaer().scaleNum);
			allImage.putAll(manager.ImageManager.getImageMnagaer().getImagesMapByImageConfig(manager.ImageManager.getImageMnagaer().createImageConfigByPlist("cannon/fire"), manager.ImageManager.getImageMnagaer().scaleNum));
			//初始化金币数字
			...
			initNet(allImage);
			//初始化水波纹
			initWaterRipple(manager.ImageManager.getImageMnagaer().getImagesMapByImageConfig(manager.ImageManager.getImageMnagaer().createImageConfigByPlist("cannon/ripple"), manager.ImageManager.getImageMnagaer().scaleNum));
			//初始化大炮
			initCannon(allImage);
			//初始化激光
	...
	private void initGoldNum(HashMap<String,Bitmap> allImage){
		...
		manager.ScoreManager.getScoreManager().setGoldNum(imgs);
	}
	/**
	 * 初始化更换大炮的效果图
	 */
	private void initChangeCannonEffect(){
		HashMap<String,Bitmap> allEffect = manager.ImageManager.getImageMnagaer().getImagesMapByImageConfig(manager.ImageManager.getImageMnagaer().createImageConfigByPlist("cannon/changefire"), manager.ImageManager.getImageMnagaer().scaleNum);
	...
	public void initCannon(){
		...
		manager.LayoutManager.getLayoutManager().initCannon(getCannon(currentCannonIndex));
		setShotable(true);
	}
	/**
	 * 初始化渔网
	...
	public void upCannon(){
		...
		//播放更换大炮的音效
		manager.SoundManager.playSound(manager.SoundManager.SOUND_BGM_CHANGE_CANNON);
		manager.LayoutManager.getLayoutManager().updateCannon(getCannon(currentCannonIndex));
	...
	public void shot(float targetX,float targetY){
		if(shotable){
			...
			if(GamingInfo.getGamingInfo().getScore()>=currentCannonIndex){
				...
				//开炮的声音
				manager.SoundManager.playSound(manager.SoundManager.SOUND_BGM_FIRE);
				...
				//没有金币的声音
				manager.SoundManager.playSound(manager.SoundManager.SOUND_BGM_NO_GOLD);
